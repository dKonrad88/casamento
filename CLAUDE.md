...
- **Home (`homeHTML`):** hero "Diego & Débora / Nosso casamento / data", fio dourado, **contagem** ("N dias para o grande dia") — **sem card/fundo e sem o coração**, número grande em Playfair centrado sobre o texto, tudo no centro da tela, entre dois fios `.rule`; é só informação, **não é clicável**. O fio de baixo faz parte do bloco da contagem (some junto quando não há data, senão sobrariam dois fios colados) —, faixa **`.figs.duo`** com Projeção × Meta (mesmo formato sóbrio da tela de Gastos; Meta vira "—" se não definida) e 4 tiles. O tile de Gastos mostra "R$ X a pagar" (projeção/meta já aparecem na faixa acima). A tela `inicio` continua existindo, mas **nada na home linka pra ela** — só a data quando não está definida (`setdata`).
...
- **Convidados (`convListaHTML`/`renderConvidados`)** — a tela mais trabalhada:
  ...
  - **Modo de entrega (`c.via`)**: ícone antes do avião — **envelope** = convite físico (em mãos), **celular** = online, **cinza** = ainda a definir. Tocar abre o bottom-sheet `formVia` com as duas opções (+ "Deixar a definir" quando já escolhido); `cviapick` abre, `cvia` grava. Campo é **removido** do objeto ao limpar (não vira string vazia). O resumo do topo conta "N físicos · M online · K a definir" — é o que diz quantos convites imprimir. `persistConvidado` não mexe em `via`, então editar o convidado preserva a escolha.
...
- **Gastos** — paridade funcional com o HUB + tratamento editorial (feito depois de Convidados):
  ...
  - **Lista:** barra `.grp` marrom com o nome do filtro + contagem. Cada linha é `.sw-content.gsw` — **cor só no dot** de 8px (`casStatusColor`), texto todo em tinta/cinza. Título e sub **quebram em quantas linhas precisarem** (o Diego pediu ver o nome inteiro — antes era 1 linha com ellipsis, o que cortava "Cerimonialista — Al…"); o **status textual só aparece em Planejando ou se cancelado** (em Fechados o dot já diz). Datas curtas dd/mm/aa (`fmtBRc`).
